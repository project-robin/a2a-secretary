...
export function ChatStream({
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-stone-50/30 overflow-hidden relative">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 md:p-10 space-y-6 md:space-y-8 scroll-smooth"
      >
        {messages.length === 0 && (
        ...
        {messages.map((msg, i) => (
          ...
          >
            <div
              className={`
                max-w-[90%] md:max-w-[70%] px-4 py-3 md:px-6 md:py-4 rounded-[1.5rem] md:rounded-[2rem] text-sm font-medium leading-relaxed
                ${msg.role === "user"
        ...
        {isLoading && chatMessages.length === 0 && (
          <div className="flex justify-start animate-pulse">
            <div className="bg-white border border-stone-200 px-4 py-3 md:px-6 md:py-4 rounded-[1.5rem] md:rounded-[2rem] rounded-tl-none flex items-center gap-2 md:gap-3">
      ...
      <div className="p-4 md:p-8 bg-white/80 backdrop-blur-md border-t border-stone-200 sticky bottom-0">
        <div className="max-w-4xl mx-auto relative">
          ...
          <div className="relative group">
            <input
              ...
              className="w-full bg-stone-100 border-none rounded-2xl px-4 md:px-8 py-4 md:py-5 pr-14 md:pr-16 text-sm font-medium focus:ring-4 focus:ring-stone-900/5 transition-all outline-none"
              disabled={isLoading}
            />
            <button
              onClick={handleSendMessage}
              disabled={isLoading || !input.trim()}
              className="absolute right-2 top-2 bottom-2 aspect-square bg-stone-900 text-white rounded-xl flex items-center justify-center hover:bg-stone-800 transition-all disabled:opacity-20 active:scale-90 shadow-lg shadow-stone-900/20"
            >
